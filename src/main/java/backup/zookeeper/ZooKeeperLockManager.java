...
public class ZooKeeperLockManager implements Closeable {
  ...
  public void unlock(String name) throws InterruptedException, KeeperException {
    ...
    LOG.debug("Unlocking on path [{}] with name [{}]", lockPath, name);
  ...
  public boolean tryToLock(String name) throws KeeperException, InterruptedException {
    ...
    List<String> children = getOnlyThisLocksChildren(name, _zooKeeper.getChildren(_lockPath, false));
    Collections.sort(children);
    String firstElement = children.get(0);
    if ((_lockPath + "/" + firstElement).equals(newPath)) {
      // yay!, we got the lock
      LOG.debug("Lock on path [{}] with name [{}]", _lockPath, name);
      return true;
    } else {
      LOG.debug("We did not get lock for path [{}] with name [{}]", _lockPath, name);
    ...
    while (true) {
      synchronized (_lock) {
        ...
        if ((_lockPath + "/" + firstElement).equals(newPath)) {
          ...
          LOG.debug("Waiting for lock on path [{}] with name [{}]", _lockPath, name);
          _lock.wait(_timeout);
        }
      }
    }
